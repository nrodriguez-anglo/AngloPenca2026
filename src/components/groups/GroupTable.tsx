...
const positionColors: Record<number, string> = {
  ...
  3: 'border-l-2 border-l-zinc-400',     // posible mejor 3ro
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="text-zinc-200 text-xs uppercase tracking-wide border-b border-border">
            ...
            <th className="text-center pb-2 px-2 font-bold text-zinc-100">PTS</th>
          </tr>
        </thead>
        <tbody>
          {standings.map((row) => (
            <tr
              ...
              className={`${positionColors[row.position]} transition-colors ${
                onTeamClick ? 'cursor-pointer hover:bg-white/20' : 'hover:bg-zinc-200'
              }`}
            >
              <td className="py-2 pl-3 text-zinc-300 text-xs">{row.position}</td>
              ...
              <td className="py-2 pl-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-100 text-xs font-medium truncate max-w-[120px]">
                    {row.is_confirmed
                      ? row.team_name
              ...
              <td className="py-2 px-2 text-center text-zinc-300">{row.pj}</td>

              {!compact && (
                <>
                  <td className="py-2 px-2 text-center text-zinc-300">{row.pg}</td>
                  <td className="py-2 px-2 text-center text-zinc-300">{row.pe}</td>
                  <td className="py-2 px-2 text-center text-zinc-300">{row.pp}</td>
                  <td className="py-2 px-2 text-center text-zinc-300">{row.gf}</td>
                  <td className="py-2 px-2 text-center text-zinc-300">{row.gc}</td>
                </>
              )}
              ...
              <td className="py-2 px-2 text-center font-bold text-zinc-300">{row.pts}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {/* Leyenda */}
      {!compact && (
        <div className="flex bg-blackitems-center gap-4 mt-3 px-1 text-[10px] text-zinc-200">
